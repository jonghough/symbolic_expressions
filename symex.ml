...
let rec (+:) e d =
	...
	| Int a, Int b					-> Int (a + b)
	| Int 0, a | a, Int 0				-> a
	| Int a, (Neg (Int b)) | (Neg (Int b)), Int a 	-> Int (a - b)
...
and diff e1 =
	...
	| Mul(X, X)					-> Mul((Int 2), X)
	| Mul(a, Add(b,c)) | Mul(Add(b,c), a)		-> Add(diff (Mul(a,b)),(diff(Mul(a,c))))
	...
	| Log a						-> Div((diff a), a)
        ...
        | Csc a						-> Mul((diff a), Neg( Mul(Csc a, Cot a)))
        | Sec a                             		-> Mul((diff a), Mul(Sec a, Tan a))
        | Cot a                             		-> Mul ((diff a), Neg (Pow(Csc a, Int 2)))
        | a						-> a
...
and handle_integral_of_mult a b =
		...
		| Pow(X, p), q | q, Pow(X, p)		-> Sub(Mul(Pow(X,p), integral q),integral(simplify( Mul(integral q, simplify(Pow(X, Sub(p,Int 1)))))))
        | a, b						-> Itg(Mul(a,b))
...
and integral e =
        ...
        | X						-> Pow(X, Int 2)
        | E X                               		-> E X
        | E (Mul(a, X)) when contains X a = false       -> Div(E(Mul(a,X)),a)
        | E(Mul(X, a)) when contains X a = false        -> Div(E(Mul(a,X)),a)
        | E(Add(X, a)) when contains X a = false        -> integral (E(Add(a,X)))
        | E(Add(a,X)) when contains X a = false         -> E(Add(a,X))
        | E (Neg X)					-> Neg(E(Neg X))
        ...
        | Sin X						-> Neg(Cos X)
        | Sin(Mul(a,X)) when contains X a = false       -> Div(Neg(Cos(Mul(a,X))),a)
        | Sin(Mul(X,a)) when contains X a = false       -> Div(Neg(Cos(Mul(a,X))),a)
        | Cos X                             		-> Sin X
        | Tan X                             		-> Log(Sec X)
...
and simplify e = 
	...
	| Mul(a, Div(b,c)) | Mul(Div(b,c),a)		-> Div(simplify(Mul(a,b)),simplify c)
	...
	| Mul(Pow(a, Int b),Pow(c, Int d)) when a = c && b=(-1*d)			
							-> Int 1
	| Mul(E (Int a),E (Int b)) when a=(-1*b)	-> Int 1
